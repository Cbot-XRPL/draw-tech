...
async function analyzePreviewForLayerPrompts(apiKey, previewDataUrl, layers, canvas) {
  const prompt = [
    ...
    "- EXACT WIDTH-TO-HEIGHT RATIO of this element as it appears in the preview (e.g. 'roughly square 1:1', 'wider than tall at about 1.3:1', 'taller than wide at about 0.8:1')",
    "- EXACT CANVAS COVERAGE — what percentage of the canvas width and height does this element span? (e.g. 'spans about 60% of the canvas width and 75% of the canvas height')",
    "- EXACT POSITION — where is the center of this element? (e.g. 'centered horizontally, sitting in the lower 60% of the canvas with top at roughly 20% from top')",
    ...
    "",
    "For the BASE/BODY layer — THIS IS THE MOST CRITICAL LAYER. Every other layer depends on it being correct:",
    "- MUST match the EXACT width-to-height ratio from the preview. If the vault is roughly square, it must be square. If it's wider than tall, it must be wider than tall by the same ratio.",
    "- MUST match the EXACT 3D rotation angle. Measure the turn precisely — is it 15 degrees? 30 degrees? 45 degrees? State the exact angle.",
    "- MUST match the EXACT canvas coverage. If the body fills 65% width and 80% height in the preview, say so explicitly.",
    "- MUST match the EXACT vertical position. If the body sits in the lower portion with space above for a topper, say so.",
    "- Strip ALL accessories but keep the base structural form EXACTLY as proportioned in the preview.",
    "- Do NOT flatten, widen, shorten, or re-proportion the body in any way.",
    "",
