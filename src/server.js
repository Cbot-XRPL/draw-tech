...
function buildStructuralChildPrompt(childLayerName, baseLayerName, spec) {
  ...
  return (
    ...
    ` EDGE STYLE: Draw the ${childLayerName} with thin, flush edges — NOT thick extruded 3D bevels. The rim and border should feel like a flat or gently raised lip that sits flush against the ${baseLayerName}'s surface, not a chunky protruding frame. Keep edge thickness minimal so the ${childLayerName} looks like it tucks INTO the opening rather than sitting proud on top of it. Avoid heavy drop shadows or deep 3D extrusion on the outer border — the edge should blend seamlessly where it meets the ${baseLayerName}.` +
